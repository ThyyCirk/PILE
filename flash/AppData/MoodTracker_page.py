...
class MoodTracker_page:
    ...
    def page0(self):
        ...
        for i in range(valFrom, valTo + 1):
            key=f"{i}_{t[1]}_{t[0]}"
            value=FileManager.search(key, '/flash/libs/moodTracker.json')
            if value != None:
                insertV = [key, value, x]
    ...
    def pageSetMood(self, confirm=False):
        ...
        if confirm:
            FileManager.write(f"{t[2]}_{t[1]}_{t[0]}", f"{self.MoodTracker_Pile}", '/flash/libs/moodTracker.json')
            self.MoodTracker_Num = 0
            GlobalVars.ALLOW_SETTINGS_PAGE = True
            GlobalVars.ALLOW_NEXT_PAGE = True
            GlobalVars.OVERRIDE_NEXT_PAGE_BTN = False 
    ...
    def pageMonthFeeling(self):
        ...
        for i in range(1, to):
            ...
            valueprev=FileManager.search(keyprev, '/flash/libs/moodTracker.json')
