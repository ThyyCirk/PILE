...
class FileManager:
    ...
    def _load(file):
        ...
        try:
            with open(file) as f:
                try:
                    FileManager._cache[file] = json.load(f)
                except ValueError:
                    FileManager._cache[file] = {}
        except OSError:
    ...
    @staticmethod
    def search(name, file="/flash/libs/data.json"):
        FileManager._load(file)
        return FileManager._cache[file].get(name)

    @staticmethod
    def write(name, new_value, file="/flash/libs/data.json"):
        FileManager._load(file)
        FileManager._cache[file][name] = new_value
    ...
    def _flush(file):
        ...
        with open(tmp_name, "w") as f:
            json.dump(FileManager._cache[file], f)
        try:
            os.remove(file)
        except OSError:
            pass
